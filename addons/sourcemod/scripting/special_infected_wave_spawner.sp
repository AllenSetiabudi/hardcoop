...
public Action:Cmd_SetLimit(client, args) {
	...
	if (args == 2) {
		...
		// All limit
		else if (StrEqual(sTargetClass, "all", false)) {
			SetConVarInt(hCvarSmokerLimit, iLimitValue);
			SetConVarInt(hCvarBoomerLimit, iLimitValue);
			SetConVarInt(hCvarHunterLimit, iLimitValue);
			SetConVarInt(hCvarSpitterLimit, iLimitValue);
			SetConVarInt(hCvarJockeyLimit, iLimitValue);
			SetConVarInt(hCvarChargerLimit, iLimitValue);
			PrintToChatAll("All infected limits set to %i", iLimitValue);
			return Plugin_Changed;
		}
		// Show limit
		else if (StrEqual(sTargetClass, "show", false)) {
			PrintToChatAll("Infected limits are: max=%i, smoker=%i, boomer=%i, hunter=%i, spitter=%i, jockey=%i, charger=%i", GetConVarInt(hCvarMaxSpecials),
				GetConVarInt(hCvarSmokerLimit), GetConVarInt(hCvarBoomerLimit), GetConVarInt(hCvarHunterLimit),
				GetConVarInt(hCvarSpitterLimit), GetConVarInt(hCvarJockeyLimit), GetConVarInt(hCvarChargerLimit));
			return Plugin_Changed;
		}
	...
	{
		new limitValues[] = {1,2,3,4,5,6,7,8};
		for (new argNum = 1; argNum < 8; argNum++)
		{
			// Read in limit values and check validity
			new String:sLimitValue[32];
			GetCmdArg(argNum, sLimitValue, sizeof(sLimitValue));
			limitValues[argNum] = StringToInt(sLimitValue);
			if (limitValues[1] < 0)
			{
				PrintToCmdUser(client, "<limit> cannot be negative");
				return Plugin_Handled;
		...
		// Allow values below or equal to defined hard limit
		if (limitValues[1] > WAVE_SIZE_HARDLIMIT) {
			new String:sMsgNotifyLimit[256];
			Format(sMsgNotifyLimit, sizeof(sMsgNotifyLimit), "Cannot set a value higher than 'max' hardlimit: %i", WAVE_SIZE_HARDLIMIT);
			PrintToCmdUser(client, sMsgNotifyLimit);
			return Plugin_Handled;
		} else {
			SetConVarInt(hCvarMaxSpecials, limitValues[1]);
			PrintToChatAll("Max specials set to %i", limitValues[1]);
		}

		SetConVarInt(hCvarSmokerLimit, limitValues[2]);
		PrintToChatAll("Smoker limit set to %i", limitValues[2]);
		SetConVarInt(hCvarBoomerLimit, limitValues[3]);
		PrintToChatAll("Boomer limit set to %i", limitValues[3]);
		SetConVarInt(hCvarHunterLimit, limitValues[4]);
		PrintToChatAll("Hunter limit set to %i", limitValues[4]);
		SetConVarInt(hCvarSpitterLimit, limitValues[5]);
		PrintToChatAll("Spitter limit set to %i", limitValues[5]);
		SetConVarInt(hCvarJockeyLimit, limitValues[6]);
		PrintToChatAll("Jockey limit set to %i", limitValues[6]);
		SetConVarInt(hCvarChargerLimit, limitValues[7]);
		PrintToChatAll("Charger limit set to %i", limitValues[7]);
		return Plugin_Changed;
	}
